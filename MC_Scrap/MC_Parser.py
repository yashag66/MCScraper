...
from utilities_py import CURRENT_PATH, DOWNLOAD_DIR, ROOT_DIR, DOWNLOAD_DIR_PROCESSED
from db_util import Session, Stock, Stock_Details
from db_util import set_stock_details
from utilities_py import stocks_info_mapping, move_file
...
def getDataForRest(file=None):
    with open(file, 'r', encoding='utf-8') as f_read:
        ...
        if 'exception' not in str(stock_quotes).lower():
            ...
            stocks_detail_mapping = stocks_info_mapping(stock_quotes, temp_symbol)
            set_stock_details(class_ref=Stock_Details, data=stocks_detail_mapping, symbol=temp_symbol)
    move_file(SOURCE_DIR_FILE=[file], DEST_DIR=DOWNLOAD_DIR_PROCESSED)
