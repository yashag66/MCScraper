...
from utilities_py import current_date_time, CURRENT_PATH, DOWNLOAD_DIR_PRE_PATH, ROOT_DIR

directory_create(DOWNLOAD_DIR_PRE_PATH)
SYMBOL_WEBPAGE_MAPPING = []
...
if __name__ == '__main__':
    filepath = os.path.join(CURRENT_PATH, 'Stocks_Info', 'MC_Link.csv')
    stocks_link = pd.read_csv(filepath, header = [0])
    for stocks_details in stocks_link.to_dict('records'):
        print(stocks_details['symbol'], stocks_details['mc_link'])
        stock_symbol = stocks_details['symbol']
        directory_create(os.path.join(DOWNLOAD_DIR_PRE_PATH, stock_symbol))
        main(os.path.join(DOWNLOAD_DIR_PRE_PATH, stock_symbol), stocks_details['mc_link'], stock_symbol)
