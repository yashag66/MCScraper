...
class Stock_Details(BaseMixin, Base):
    ...
    def set_details(self, details={}):
        ...
        """
        print(details)
        self.stock_id = int(details['stock_id'])
        self.open_price = float(details['open_price'])
        self.prev_close_price = float(details['prev_close_price'])
        self.open_price = float(details['open_price'])
        self.high_price = float(details['high_price'])
        self.low_price = float(details['low_price'])
        self.average_price = float(details['average_price'])
        self.no_of_contracts_traded = int(details['no_of_contracts_traded'])
        self.turnover_in_lakhs = float(details['turnover_in_lakhs'])
        self.market_lot = int(details['market_lot'])
        self.open_interest = float(details['open_interest'])
        self.open_interest_change = float(details['open_interest_change'])
        self.open_interest_change_percent = float(details['open_interest_change_percent'])
    ...
    def __iter__(self):
        ...
        temp['date'] = datetime(self.date).date
        for key, value in temp.items():
            yield (key, value)
    ...
    if class_ref:
        if class_ref == Stock_Details:
            ...
            try:
                ...
                data['stock_id'] = dict(stock_mappings)['id']
                try:
                    temp_stock_details = Stock_Details()
                    temp_stock_details.set_details(data)
                    session.add(temp_stock_details)
                    session.commit()
                except Exception as e:
