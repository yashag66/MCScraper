...
from db_util import Session, Stock, get_all_stocks

SYMBOL_WEBPAGE_MAPPING = []
session = Session()

for stock_info in get_all_stocks(Stock):
    SYMBOL_WEBPAGE_MAPPING.append({stock_info['symbol']:stock_info['mc_link']})
...
if __name__ == '__main__':
    for stock_info in SYMBOL_WEBPAGE_MAPPING:
        for stock_symbol, stock_link in stock_info.items():
            directory_create(os.path.join(DOWNLOAD_DIR, stock_symbol))
            main(os.path.join(DOWNLOAD_DIR, stock_symbol), stock_link, stock_symbol)
